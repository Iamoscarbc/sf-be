...
class Admin extends CI_Controller {
	...
	// WATCH LIST OF MOVIES, MANAGE THEM
	function movie_resize($actor_id = "")
	{
		$page_data['actor_id']		=	empty($actor_id) ? 'all' : $actor_id;
		$page_data['page_name']		=	'movie_list';
		$page_data['page_title']	=	'Manage movie';
		...
		$counter = 1;
		$configs = array();
		$target_path = $_SERVER['DOCUMENT_ROOT'] . '/assets/global/movie_thumb/thumbnail/';
        foreach ($movies as $row){
			//echo $row['title'].'<br>';

			$config['image_library'] = 'gd2';
			$config['source_image'] = './'.$this->get_thumb_file_url('movie' , $row['movie_id']);
	...
	function get_thumb_file_url($type = '' , $id = '')
	{
        if (file_exists('assets/global/'.$type.'_thumb/' . $id . '.jpg'))
            $image_url = 'assets/global/'.$type.'_thumb/' . $id . '.jpg';
        else
            $image_url = '';
